...
import './PopularReposCard.css';

interface PopularReposCardProps {
  data: Data;
}
...
export const PopularReposCard: FC<PopularReposCardProps> = memo(({ data }) => {
  ...
  return (
    <Card
      ...
      className="popular-repos-card"
      titleSlot={<ButtonGroup labels={MODES} values={[...MODES]} onValueChanged={setMode} />}
    >
      <div className="popular-repos-card__list">
        {items.map((item, i) => {
          const content = (
            <>
              <div
                className="popular-repos-card__color"
                style={{ '--color': `var(--${item.color})` } as React.CSSProperties}
              />
              <div className="popular-repos-card__info">
                <div className="popular-repos-card__name">{item.name}</div>
                <div className="popular-repos-card__description">{item.description || '\u00A0'}</div>
              </div>
              <div className="popular-repos-card__bar">
                <Bar
                  sections={
                    item.value === maxCount
              ...
              <div className="popular-repos-card__count">
          ...
          return item.url ? (
            <a key={i} className="popular-repos-card__item" href={item.url} target="_blank" rel="noreferrer">
              {content}
            </a>
          ) : (
            <div key={i} className="popular-repos-card__item">
              {content}
            </div>
          );
        })}
