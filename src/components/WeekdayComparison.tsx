...
export default function WeekdayComparison({
  weekdays,
}: WeekdayComparisonProps) {
  const maxSum = Math.max(
    ...Object.values(weekdays.sum).map((counts) => counts.commitCount),
  );
    ...
    const sections: DataPoint[] = [
      {
        color: 'color-open',
        title: 'Open Source',
        value: weekdays.open[key].commitCount,
      },
      {
        color: 'color-closed',
        title: 'Private',
        value: weekdays.closed[key].commitCount,
      },
    ];
    bars.push(
      <Bar
        ...
        style={{
          height: `${((weekdays.sum[key].commitCount / maxSum) * 125).toFixed(0)}px`,
        }}
      />,
    );
  }

  const xAxisLabels = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(
    (label, i) => <span key={i}>{label}</span>,
  );
  ...
  return (
    <Card
      ...
      footerSlot={
        <Legend className="weekday-comparison__legend" sections={sections} />
      }
    >
